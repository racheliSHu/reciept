...
//             const result = await axios.get("https://localhost:44391/api/AnalyzeReceipt");
//             AllReceipt=result.data;
//             setRows(AllReceipt.products);// setRows(result.data);
//         }
//         getResults()
//     }, [])
...
export default function ColumnsGrid({AllReceipt}) {
    const { register, handleSubmit, setError } = useForm();
    const dateReceiptRef = useRef('20/02/2022')
    const getColumns = () => {
        return [
    ...
    //   const { setRows, setRowModesModel }
